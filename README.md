...
### Developer comment on task completion

	* Input validation has been enforced in controller level. In general all int should be positive, booking date cannot be in the past (calendar view date can be in the past). PreparationTimeInDays can be 0 or more.
	
	* Implemented async call all the way from controller to data layer. In reality data related tasks are asyc request. For now, all data tasks are off-loaded to a task to mimic async.
	
	* Unit tests were added largely to test unit of business functionality. Left existing unit tests alone whereever possible. Because of added properties (PreparationTimeInDays & Unit), and date validation, tests had to be modified accordingly.
	
	* In real life scenario, error messages sent as part of internal server errors should be more generic without providing specifics (stacks, objects, variables etc). Exceptions should be logged on the server end with details.